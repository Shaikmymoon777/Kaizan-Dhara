...
const HistorySidebar: React.FC<HistorySidebarProps> = ({
    ...
    const loadHistory = async () => {
        try {
            ...
            if (response.ok) {
                ...
                // Map backend data to HistoryItem format if needed
                // Backend returns Project[], HistoryItem expects { id, name, prompt, timestamp, project }
                // We need to adapt the response
                const adaptedHistory: HistoryItem[] = historyData.map((p: any) => ({
                    id: p.id,
                    name: p.name,
                    prompt: p.prompt,
                    ...
                    project: {
                        ...
                        design: p.design ? JSON.parse(p.design) : undefined,
                        tests: p.tests ? JSON.parse(p.tests) : undefined,
                    }
                }));
                setHistory(adaptedHistory);
                setFilteredHistory(adaptedHistory);
            }
        } catch (error) {
    ...
    const handleDelete = async (id: string, e: React.MouseEvent) => {
        e.stopPropagation();
        // Backend delete not yet implemented in ProjectController, for now we just remove from UI or implement delete endpoint
        // Assuming we will add delete to backend, but task didn't explicitly ask for Delete API. 
    ...
    const handleClearAll = async () => {
        if (window.confirm('Wipe all project memory? This action is irreversible.')) {
            alert("Clear all functionality coming soon to backend.");
        }
    };
